...
open class AZImagePresenterViewController: UIViewController {
    ...
    fileprivate var center: CGPoint? {
        if let originalImage = originalImage,
            let superView = delegate?.previewImageViewInRespectTo(originalImage) {
    ...
    @objc internal func handleTap(_ sender: UITapGestureRecognizer) {
        UIView.animate(withDuration: animationDuration) {
            if self.isZoomedIn {
                ...
                // zoom in
                self.scrollView.zoomScale = (min(self.scrollView.minimumZoomScale * 4.0,
    ...
    fileprivate func dismissDirection(_ baseView: TrackableImage, scale: CGFloat, finalPoint: CGPoint) {
        ...
        baseView.addCornerRadiusAnimation(from: baseView.cornerRadius,
                                          to: originalImage!.cornerRadius * (1/scale),
...
extension AZImagePresenterViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidZoom(_ scrollView: UIScrollView) {
        if let view = self.scrollView.view {
            ...
            let verticalInset = viewSize.height < scrollViewSize.height ?
                (scrollViewSize.height - viewSize.height) / 2 : 0
            let horizontalInset = viewSize.width < scrollViewSize.width ?
                (scrollViewSize.width - viewSize.width) / 2 : 0
...
fileprivate final class ObjectAssociation<T: Any> {
    ...
    public subscript(index: AnyObject) -> T? {
        ...
        get {
            guard let key = objc_getAssociatedObject(index, Unmanaged.passUnretained(self).toOpaque()) as? T? else {
                return nil
            }
            return key }
...
private class FixedNavigationController: UINavigationController {
