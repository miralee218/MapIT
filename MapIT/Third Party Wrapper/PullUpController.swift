...
open class PullUpController: UIViewController {
    ...
    fileprivate func addInternalScrollViewPanGesture() {
        internalScrollView?.panGestureRecognizer.addTarget(
            self,
    ...
    private func setPortraitConstraints(parentViewSize: CGSize, customTopOffset: CGFloat? = nil) {
        ...
        leftConstraint?.constant = (parentViewSize.width -
            min(pullUpControllerPreferredSize.width, parentViewSize.width))/2
...
extension UIViewController {
    ...
    open func addPullUpController(_ pullUpController: PullUpController,
        ...
        assert(!(self is UITableViewController),
               "It's not possible to attach a PullUpController to a UITableViewController.")
